...
public class CardService extends HostApduService {
	...
	private static final String SAMPLE_LOYALTY_CARD_AID = "F555555555";
	...
	private static final byte[] UNKNOWN_CMD_SW = HexStringToByteArray("698D");
	private static final byte[] SELECT_APDU = BuildSelectApdu(SAMPLE_LOYALTY_CARD_AID);

	NfcAdapter mNfcAdapter = null;

	static byte[] newabel_bs_working = { 'n', 'e', 'w', 'a', 'b', 'e', 'l', 0x01 };
	static byte[] newabel_bs_nomal = { 'n', 'e', 'w', 'a', 'b', 'e', 'l', 0x00 };
	private static final int MESSAGE_GET = 0;
...
//		if (mNfcAdapter == null) {
//			Toast.makeText(this, "没有NFC", Toast.LENGTH_LONG).show();
//			return;
//		} else {
//			Toast.makeText(this, "找到NFC", Toast.LENGTH_LONG).show();
//		}
		//在第一次的時候，設置Card uid
		if (AccountStorage.GetAccount(this).equals("00000000")) {
			TelephonyManager mtm = (TelephonyManager) this.getSystemService(TELEPHONY_SERVICE);
			String mimei = mtm.getDeviceId();
			String mandroidid = Secure.getString(this.getContentResolver(), Secure.ANDROID_ID);
			...
			AccountStorage.SetAccount(this, ByteArrayToHexString(cardid));
		}
	}
	...
	public byte[] processCommandApdu(byte[] commandApdu, Bundle extras) {
		...
		//Toast.makeText(this, "Received APDU: " + ByteArrayToHexString(commandApdu), Toast.LENGTH_LONG).show();
		...
		if (Arrays.equals(SELECT_APDU, commandApdu)) {
			Log.i(TAG, "Sending newabel mark");
			if(SharedPreferencesUtil.getBoolean(SharedPreferencesUtil.IS_WORK_MODE,false)){
				return ConcatArrays(newabel_bs_working, SELECT_OK_SW);
			}
			return ConcatArrays(newabel_bs_nomal, SELECT_OK_SW);
		} else if (commandApdu[0] == (byte) 0x00 && commandApdu[1] == (byte) 0x83 && commandApdu[2] == (byte) 0x00
				&& commandApdu[3] == (byte) 0x00 && commandApdu.length == 14) {
			Log.i(TAG, "Sending phone uid");
			String account = AccountStorage.GetAccount(this);
			byte[] cardid = HexStringToByteArray(account);
			byte[] rand = new byte[8];
			for(int i = 0; i < 8; i++)
				rand[i] = commandApdu[5+i];
			MainDES.initkey(rand);
			byte[] mac = MainDES.idmac(cardid);
			byte[] out = new byte[4 + 4];
			System.arraycopy(cardid, 0, out, 0, 4);
			System.arraycopy(mac, 0, out, 4, 4);				
			mHandler.obtainMessage(MESSAGE_GET, (int)commandApdu[13], 0).sendToTarget();
			return ConcatArrays(out, SELECT_OK_SW);
		} else {
			return UNKNOWN_CMD_SW;
		}
	}
	
	private void sendBroadcastToUpperLayer(byte buf) {

		Intent intent1 = new Intent("Xafd_NfcKey");
		intent1.setFlags(4096);
		String numberStr = "0000" + Integer.valueOf(buf).toString();
		intent1.putExtra("dat", numberStr);
		System.out.println("[waleyx]NFCKey send broadcast with nfc reader number=" + numberStr);
